        ...
        createFakeHttpsWebSite(
            ...
            (req, res) => {
                ...
                let proxyReq = https
                    ...
                    .on('error', err => {
                        log(
                            `https tranparent: ${options.method} ${
                                options.hostname
                ...
                if (host.protocal === 'https') {
                    let httpsReq = https
                        .request(options, httpsRes => {
                            httpsRes.headers['simple-moxcy-host'] =
                                options.hostname
                            httpsRes.headers['simple-moxcy-port'] = options.port
                            httpsRes.headers[
                                'simple-moxcy-headers'
                            ] = JSON.stringify(host.headers || {})
                        ...
                        .on('error', err => {
                            log(
                                `http => https: ${method} ${hostname}:${port}${
                                    path.split('?')[0]
                                } => ${host.protocal} ${host.ip}:${host.port} `,
                                options,
                                err
                            )
                            res.end()
                    ...
                    let proxyReq = http
                        .request(options, proxyRes => {
                            if (host) {
                                proxyRes.headers['simple-moxcy-host'] =
                                    options.hostname
                                proxyRes.headers['simple-moxcy-port'] =
                                    options.port
                                proxyRes.headers[
                                    'simple-moxcy-headers'
                                ] = JSON.stringify(host.headers || {})
                        ...
                        .on('error', err => {
                            log(
                                `http => http:${method} ${hostname}:${port}${
                                    path.split('?')[0]
                                } => ${host.protocal} ${host.ip}:${host.port} `,
                                options,
                                err
